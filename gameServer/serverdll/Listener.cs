...
namespace Server
{
    public class Listener
    {
        ...
        //This is delegate for handling clients.
        ...
        public void Start(int backlog)
        {
            ...
            if (Host == "0.0.0.0")
            {
                Console.WriteLine("IP Address not Specified.. Finding new IP Adress. ");
                address = IPAddress.Any;//if IP address is not specified, get IP adress that server is connected
            }
            else
            {
                try
            ...
            try
            {
                ...
                _acceptArgs.Completed += On_Accept_Completed;//attaching event handler
                                                             //event attatched here will be executed when client is accepted
        ...
        {
            On_Accept_Completed(null, _acceptArgs);//call the method Manuallly
        }

        public void On_Accept_Completed(object sender, SocketAsyncEventArgs acceptedArg)
        {
            ...
            else //Client Acception failed
            {
                Console.WriteLine("Client Acception failed.");
                Console.WriteLine("Error type: {0}", acceptedArg.SocketError);
            }
            _flowControlEvent.Set();
            //Control sholdn't reach here
        }

        public void Abort_Acception()//Never call this method unless emergency!
        {
            _listenThread.Abort();
        ...
        {
            Console.WriteLine("<IPEndint Info>");
